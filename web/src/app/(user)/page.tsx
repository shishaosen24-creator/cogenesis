        ...
        <main className="h-full overflow-hidden text-[color:var(--sacred-on-surface)]">
            <section className="stage-opening relative flex h-full items-center justify-center overflow-hidden px-5 pb-12 pt-20">
                ...
                <div className="relative z-10 mx-auto flex w-full max-w-[1180px] flex-col items-center text-center">
                    <img src="/brand/site-logo-transparent.png" alt="CoGenesis" className="stage-logo-reveal h-24 w-24 object-contain drop-shadow-[0_0_28px_rgba(197,160,89,0.55)] sm:h-32 sm:w-32" loading="eager" decoding="async" fetchPriority="high" />
                    ...
                    <div className="stage-reveal stage-delay-actions mt-10 flex flex-col gap-3 sm:flex-row sm:gap-5">
                        <Link className="btn-sacred h-12 px-7" href="/canvas" prefetch={false} onMouseEnter={() => warmRoute("/canvas")} onFocus={() => warmRoute("/canvas")} onMouseLeave={cancelWarmRoute} onClick={cancelWarmRoute}>
                            开始共创
                        </Link>
                        <Link className="btn-sacred h-12 px-7" href="/login" prefetch={false} onMouseEnter={() => warmRoute("/login")} onFocus={() => warmRoute("/login")} onMouseLeave={cancelWarmRoute} onClick={cancelWarmRoute}>
                            连接端点
                        </Link>
                        <Link className="btn-sacred stage-primary-link h-12 px-7" href="/canvas" prefetch={false} onMouseEnter={() => warmRoute("/canvas")} onFocus={() => warmRoute("/canvas")} onMouseLeave={cancelWarmRoute} onClick={cancelWarmRoute}>
                            <span>进入工作台</span>
                    ...
                    <nav className="stage-reveal stage-delay-nav mt-10 hidden flex-wrap justify-center gap-7 md:flex">
                        {stageNav.map((item) => (
                            <Link key={item.label} href={item.href} prefetch={false} onMouseEnter={() => warmRoute(item.href)} onFocus={() => warmRoute(item.href)} onMouseLeave={cancelWarmRoute} onClick={cancelWarmRoute} className="sacred-label text-[color:var(--sacred-on-surface-variant)] transition hover:text-[color:var(--sacred-tertiary)]">
                                {item.label}
                            </Link>
                        ))}
                    </nav>
