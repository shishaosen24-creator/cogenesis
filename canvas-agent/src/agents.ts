...
export function runClaudeTurn(prompt: string, emit: AgentEmit, attachments: AgentAttachment[] = []) {
    if (!prompt.trim()) return;
    const promptWithFiles = attachments.length ? `${prompt}\n\n用户随消息附带了 ${attachments.length} 个图片参考，请优先使用网页画布里的素材包、选区和任务队列；若需要图片细节，请要求用户先把参考图加入画布节点或素材包。` : prompt;
    const child = spawnAgent("claude", ["-p", "--output-format", "stream-json", "--verbose", "--include-partial-messages", "--allowedTools", "mcp__cogenesis-canvas__*", promptWithFiles], ["ignore", "pipe", "pipe"], emit);
    if (!child) return;
...
class CodexAppClient {
    ...
    static async start(emit: AgentEmit) {
        ...
        await client.request("initialize", { clientInfo: { name: "cogenesis-canvas-agent", title: "CoGenesis Canvas Agent", version: VERSION }, capabilities: { experimentalApi: true, requestAttestation: false } });
...
function codexConfig() {
    return { mcp_servers: { "cogenesis-canvas": { command: canvasAgentMcp.command, args: canvasAgentMcp.args, default_tools_approval_mode: "approve", startup_timeout_sec: 20, tool_timeout_sec: 90 } } };
}
...
async function writeAttachmentFile(item: AgentAttachment) {
    ...
    const file = path.join(os.tmpdir(), `cogenesis-canvas-${Date.now()}-${Math.random().toString(16).slice(2)}.${imageExt(meta || item.type)}`);
