...
export const toolNames = [
    ...
    "canvas_get_asset_pack",
    "canvas_get_task_queue",
    "canvas_get_chain_events",
    "canvas_get_director_workflows",
    ...
    "canvas_create_director_workflow",
...
export const toolInputSchemas = {
    ...
    canvas_get_asset_pack: z.object({}).passthrough(),
    canvas_get_task_queue: z.object({}).passthrough(),
    canvas_get_chain_events: z.object({}).passthrough(),
    canvas_get_director_workflows: z.object({}).passthrough(),
    ...
    canvas_create_director_workflow: z.object({ prompt: z.string(), title: z.string().optional(), x: z.number().optional(), y: z.number().optional(), autoRun: z.boolean().optional() }).merge(generationOptionsSchema),
...
export const toolDescriptions: Record<ToolName, string> = {
    ...
    canvas_get_asset_pack: "读取导演台和我的素材共享给 Agent 的素材包摘要，用于保持产品、人物、场景、Logo 和参考视频一致。",
    canvas_get_task_queue: "读取导演台拆解出的故事板/生成/审校任务队列，包含节点、依赖、执行状态和提示词。",
    canvas_get_chain_events: "读取导演台和本地 Agent 共用的 AI 操作事件流，包含创建节点、连接节点、执行状态与失败摘要。",
    canvas_get_director_workflows: "按 workflowId 读取当前网页画布里的导演工作流摘要，用于本地 Agent 并发管理多个导演任务和链路状态。",
    ...
    canvas_create_director_workflow: "根据一句话主题创建导演工作流，自动生成主题拆解、视觉圣经、首图、一致性、故事板、视频片段、剪辑缺片和审校节点。",
