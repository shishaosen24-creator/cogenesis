...
import { defaultConfig, type AiConfig, type ModelCapability, useConfigStore, useEffectiveConfig } from "@/stores/use-config-store";
...
import { createCanvasNodeSpatialIndex, queryCanvasNodeSpatialIndex } from "../utils/canvas-node-spatial-index";
import { App, Button, Dropdown, Modal } from "antd";
import { NODE_DEFAULT_SIZE, getNodeSpec } from "../constants";
import { ActiveConnectionPath, ConnectionLayer, ConnectionPath } from "../components/canvas-connections";
...
import { Minimap, type MinimapNodeItem } from "../components/canvas-mini-map";
...
import { buildCanvasResourceReferences, buildNodeMentionReferences, createCanvasResourceIndex } from "../utils/canvas-resource-references";
...
const EMPTY_CANVAS_MENTION_REFERENCES = [] as ReturnType<typeof buildNodeMentionReferences>;
const EMPTY_CANVAS_NODES: CanvasNodeData[] = [];
const EMPTY_CANVAS_CONNECTIONS: CanvasConnection[] = [];
const EMPTY_CANVAS_SELECTED_NODE_IDS: string[] = [];
const EMPTY_AGENT_ASSET_PACK: CanvasAgentAssetPackItem[] = [];
const EMPTY_AGENT_TASK_QUEUE: CanvasAgentTaskQueueItem[] = [];
const EMPTY_AGENT_CHAIN_EVENTS: AIChainEvent[] = [];
const EMPTY_AGENT_VIEWPORT: ViewportTransform = { x: 0, y: 0, k: 1 };
...
function CanvasRefreshShell({ label = "正在恢复画布..." }: { label?: string }) {
    return (
        <main className="relative h-full min-h-0 overflow-hidden bg-transparent text-foreground">
            ...
            <div className="absolute bottom-5 left-1/2 z-50 flex h-14 -translate-x-1/2 items-center gap-1 rounded-xl border px-2 shadow-lg backdrop-blur" style={{ background: "var(--sacred-glass-bg)", borderColor: "var(--border)" }} aria-hidden="true">
            ...
            <div className="absolute bottom-5 left-5 z-50 flex h-14 w-[260px] items-center gap-2 rounded-xl border px-2 shadow-lg backdrop-blur" style={{ background: "var(--sacred-glass-bg)", borderColor: "var(--border)" }} aria-hidden="true">
...
function ConnectionCreateMenu({
    ...
    const minTop = 72;
    const bottomSafeArea = windowSize.width <= 480 ? 150 : edgePadding;
    const menuWidth = Math.min(300, Math.max(240, windowSize.width - edgePadding * 2));
    const menuHeightEstimate = 400;
    const maxMenuHeight = Math.max(220, windowSize.height - minTop - bottomSafeArea);
    const screenX = viewport.x + pending.position.x * viewport.k;
    const screenY = viewport.y + pending.position.y * viewport.k;
    const clampedScreenX = clamp(screenX, edgePadding, Math.max(edgePadding, windowSize.width - menuWidth - edgePadding));
    const clampedScreenY = clamp(screenY, minTop, Math.max(minTop, windowSize.height - Math.min(menuHeightEstimate, maxMenuHeight) - bottomSafeArea));
    const inverseScale = 1 / Math.max(viewport.k, 0.05);
    const menuStyle = {
        left: (clampedScreenX - viewport.x) / viewport.k,
        top: (clampedScreenY - viewport.y) / viewport.k,
        width: menuWidth,
    ...
    return (
        <div
            className="canvas-connection-create-menu thin-scrollbar absolute z-[120] overflow-y-auto rounded-[18px] border p-3 shadow-2xl backdrop-blur"
...
function ConnectionCreateOption({ theme, icon, title, description, onClick }: { theme: (typeof canvasThemes)[keyof typeof canvasThemes]; icon: React.ReactNode; title: string; description?: string; onClick?: () => void }) {
    return (
        <button type="button" className="canvas-connection-create-option flex h-16 w-full cursor-pointer items-center gap-3 rounded-2xl px-3 text-left transition" style={{ color: theme.node.text }} onClick={onClick} aria-label={title} onMouseEnter={(event) => (event.currentTarget.style.background = theme.node.fill)} onMouseLeave={(event) => (event.currentTarget.style.background = "transparent")}>
...
function InfiniteCanvasPage() {
    ...
    const mouseWorldRafRef = useRef<number | null>(null);
    const pendingMouseWorldRef = useRef<Position | null>(null);
    ...
    const visibleInteractionNodesRef = useRef<CanvasNodeData[]>([]);
    ...
    const markNodeRunning = useCallback((nodeId: string, running: boolean) => {
        setRunningNodeIds((current) => {
            ...
            if (running) next.add(nodeId);
            else next.delete(nodeId);
            return next;
        });
    }, []);

    const clearRunningNodes = useCallback((ids: Set<string>) => {
        setRunningNodeIds((current) => {
    ...
    const scheduleMouseWorld = useCallback((position: Position) => {
        pendingMouseWorldRef.current = position;
        if (mouseWorldRafRef.current) return;
        mouseWorldRafRef.current = requestAnimationFrame(() => {
            mouseWorldRafRef.current = null;
            if (!pendingMouseWorldRef.current) return;
            setMouseWorld(pendingMouseWorldRef.current);
            pendingMouseWorldRef.current = null;
        });
    }, []);

    const commitNodes = useCallback((updater: (current: CanvasNodeData[]) => CanvasNodeData[]) => {
    ...
    const appendNode = useCallback((node: CanvasNodeData) => {
        commitNodes((prev) => [...prev, node]);
    }, [commitNodes]);

    const commitConnections = useCallback((updater: (current: CanvasConnection[]) => CanvasConnection[]) => {
        const next = updater(connectionsRef.current);
    ...
    const getConnectionDropTarget = useCallback(
        (clientX: number, clientY: number, current: ConnectionHandle): ConnectionDropTarget => {
            ...
            [...visibleInteractionNodesRef.current]
                .reverse()
                .forEach((node) => {
    ...
    const hiddenBatchChildIds = useMemo(() => getHiddenBatchChildIds(nodes, collapsingBatchIds), [collapsingBatchIds, nodes]);
    const nodeSpatialIndex = useMemo(() => createCanvasNodeSpatialIndex(nodes), [nodes]);
    const visibleNodes = useStableNodeArray(useMemo(() => {
        const padding = 280;
        const rect = containerRef.current?.getBoundingClientRect();
        ...
        return queryCanvasNodeSpatialIndex(nodeSpatialIndex, { left: viewLeft, top: viewTop, right: viewRight, bottom: viewBottom }, hiddenBatchChildIds);
    }, [hiddenBatchChildIds, nodeSpatialIndex, size.height, size.width, viewport.k, viewport.x, viewport.y]));
    const visibleNodeIds = useStableStringSet(useMemo(() => new Set(visibleNodes.map((node) => node.id)), [visibleNodes]));
    const visibleInteractionNodes = useMemo(() => nodes.filter((node) => !hiddenBatchChildIds.has(node.id)), [hiddenBatchChildIds, nodes]);
    useLayoutEffect(() => {
        visibleInteractionNodesRef.current = visibleInteractionNodes;
    }, [visibleInteractionNodes]);
    const minimapItems = useMemo<MinimapNodeItem[]>(
        () =>
            isMiniMapOpen
                ? visibleInteractionNodes.map((node) => ({
                      id: node.id,
                      type: node.type,
                      x: node.position.x,
                      y: node.position.y,
                ...
                : [],
        [isMiniMapOpen, visibleInteractionNodes],
    );
    ...
    const connectionByVisibleCandidateId = useMemo(() => new Map(connections.map((connection) => [connection.id, connection])), [connections]);
    const connectionOrderById = useMemo(() => new Map(connections.map((connection, index) => [connection.id, index])), [connections]);
    ...
    const batchChildIndexByRootId = useMemo(() => {
        const map = new Map<string, Map<string, number>>();
        nodes.forEach((node) => {
            if (!node.metadata?.isBatchRoot) return;
            map.set(node.id, new Map((node.metadata.batchChildIds || []).map((childId, index) => [childId, index])));
        });
        return map;
    }, [nodes]);
    const batchMotionById = useMemo(() => {
        ...
        return map;
    }, [batchChildIndexByRootId, nodeById, visibleNodes]);
    ...
    const visibleNodePresentation = useMemo(
        () =>
            visibleNodes.map((node) => ({
                node,
                compactContent: shouldUseCompactNodeContent(
                    node,
                    viewport.k,
                    selectedNodeIds.has(node.id),
                    relatedHighlight.nodeIds.has(node.id),
                    activeNodeId === node.id,
                    connectionTargetNodeId === node.id,
                    Boolean(connectingParams),
                    dialogNodeId === node.id && !selectionBox,
                    editingNodeId === node.id,
                ),
            })),
        [activeNodeId, connectingParams, connectionTargetNodeId, dialogNodeId, editingNodeId, relatedHighlight.nodeIds, selectedNodeIds, selectionBox, viewport.k, visibleNodes],
    );
    const detailVisibleNodeIds = useStableStringSet(
        useMemo(() => new Set(visibleNodePresentation.filter((item) => !item.compactContent).map((item) => item.node.id)), [visibleNodePresentation]),
    );
    const nodeIdsRequiringDetail = useStableStringSet(
        useMemo(() => {
            const ids = new Set<string>(detailVisibleNodeIds);
            selectedNodeIds.forEach((nodeId) => ids.add(nodeId));
            [activeNodeId, dialogNodeId, editingNodeId, toolbarNodeId, infoNodeId, cropNodeId, maskEditNodeId, splitNodeId, upscaleNodeId, superResolveNodeId, angleNodeId, previewNodeId].forEach((id) => {
                if (id) ids.add(id);
            });
            return ids;
        }, [activeNodeId, angleNodeId, cropNodeId, detailVisibleNodeIds, dialogNodeId, editingNodeId, infoNodeId, maskEditNodeId, previewNodeId, selectedNodeIds, splitNodeId, superResolveNodeId, toolbarNodeId, upscaleNodeId]),
    );
    const visibleConnections = useMemo(() => {
        const ids = new Set<string>();
        visibleNodeIds.forEach((nodeId) => connectionIdsByNodeId.get(nodeId)?.forEach((connectionId) => ids.add(connectionId)));
        relatedHighlight.connectionIds.forEach((connectionId) => ids.add(connectionId));
        if (selectedConnectionId) ids.add(selectedConnectionId);

        const result: CanvasConnection[] = [];
        ids.forEach((connectionId) => {
            const connection = connectionByVisibleCandidateId.get(connectionId);
            if (!connection) return;
            const from = nodeById.get(connection.fromNodeId);
            const to = nodeById.get(connection.toNodeId);
            if (!from || !to) return;
            if (hiddenBatchChildIds.has(from.id) || hiddenBatchChildIds.has(to.id)) return;
            result.push(connection);
        });
        return result.sort((first, second) => (connectionOrderById.get(first.id) ?? 0) - (connectionOrderById.get(second.id) ?? 0));
    }, [connectionByVisibleCandidateId, connectionIdsByNodeId, connectionOrderById, hiddenBatchChildIds, nodeById, relatedHighlight.connectionIds, selectedConnectionId, visibleNodeIds]);
    const highlightedConnections = useMemo(
        ...
        [relatedHighlight.connectionIds, selectedConnectionId, visibleConnections],
    );
    const regularConnectionItems = useMemo(() => {
        return visibleConnections
            .filter((connection) => !relatedHighlight.connectionIds.has(connection.id) && selectedConnectionId !== connection.id)
            .map((connection) => {
                const from = nodeById.get(connection.fromNodeId);
                const to = nodeById.get(connection.toNodeId);
                ...
                return { id: connection.id, d: `M ${startX} ${startY} C ${startX + curvature} ${startY}, ${endX - curvature} ${endY}, ${endX} ${endY}` };
            })
            .filter((item): item is { id: string; d: string } => Boolean(item));
    }, [nodeById, relatedHighlight.connectionIds, selectedConnectionId, visibleConnections]);
    ...
    const canvasResourceIndex = useMemo(() => createCanvasResourceIndex(nodes, connections), [connections, nodes]);
    const configInputsById = useMemo(() => {
        ...
        nodeIdsRequiringDetail.forEach((nodeId) => {
            ...
            map.set(node.id, buildNodeGenerationInputs(node.id, nodes, connections, canvasResourceIndex));
        });
        return map;
    }, [canvasResourceIndex, connections, nodeById, nodeIdsRequiringDetail, nodes]);
    const canvasResourceReferences = useMemo(() => buildCanvasResourceReferences(nodes, connections, resourceContextNodeId, canvasResourceIndex), [canvasResourceIndex, connections, nodes, resourceContextNodeId]);
    ...
    const mentionReferencesByNodeId = useMemo(() => {
        ...
        nodeIdsRequiringDetail.forEach((nodeId) => {
            const node = nodeById.get(nodeId);
            if (!node) return;
            map.set(node.id, buildNodeMentionReferences(node, nodes, connections, canvasResourceIndex));
        });
        return map;
    }, [canvasResourceIndex, connections, nodeById, nodeIdsRequiringDetail, nodes]);

    const agentAssetPack = useMemo(() => (assistantVisible ? buildAgentAssetPack(nodes, sharedReferencePack, savedAssets) : EMPTY_AGENT_ASSET_PACK), [assistantVisible, nodes, savedAssets, sharedReferencePack]);
    const agentTaskQueue = useMemo(() => (assistantVisible ? buildAgentTaskQueue(nodes, connections) : EMPTY_AGENT_TASK_QUEUE), [assistantVisible, connections, nodes]);
    const agentChainEvents = useMemo(() => (assistantVisible ? buildCanvasChainEvents(nodes, connections) : EMPTY_AGENT_CHAIN_EVENTS), [assistantVisible, connections, nodes]);
        ...
        () =>
            assistantMounted
                ? {
                      projectId,
                      title: currentProject?.title || "未命名画布",
                      nodes: assistantVisible ? nodes : EMPTY_CANVAS_NODES,
                      connections: assistantVisible ? connections : EMPTY_CANVAS_CONNECTIONS,
                      selectedNodeIds: assistantVisible ? Array.from(selectedNodeIds) : EMPTY_CANVAS_SELECTED_NODE_IDS,
                      viewport: assistantVisible ? viewport : EMPTY_AGENT_VIEWPORT,
                ...
                : {
                      ...
                      connections: EMPTY_CANVAS_CONNECTIONS,
                      selectedNodeIds: EMPTY_CANVAS_SELECTED_NODE_IDS,
                      viewport: EMPTY_AGENT_VIEWPORT,
                      assetPack: EMPTY_AGENT_ASSET_PACK,
                      taskQueue: EMPTY_AGENT_TASK_QUEUE,
                      chainEvents: EMPTY_AGENT_CHAIN_EVENTS,
                  },
        [agentAssetPack, agentChainEvents, agentTaskQueue, assistantMounted, assistantVisible, connections, currentProject?.title, nodes, projectId, selectedNodeIds, viewport],
    );
    ...
    const deleteNodes = useCallback(
        (ids: Set<string>) => {
            ...
            clearRunningNodes(allIds);
            setContextMenu((current) => (current?.type === "node" && allIds.has(current.nodeId) ? null : current));
            cleanupCanvasFiles({ projectId, nodes: nodesRef.current.filter((node) => !allIds.has(node.id)), chatSessions });
        },
        [chatSessions, cleanupCanvasFiles, clearRunningNodes, projectId],
    );
    ...
    const createAndOpenProject = useCallback(() => {
        const id = createProject(`CoGenesis ${useCanvasStore.getState().projects.length + 1}`);
    ...
    const handleCanvasMouseDown = useCallback(
        (event: ReactPointerEvent<HTMLDivElement>) => {
            ...
            if (!event.ctrlKey && !event.metaKey) {
                if (selectionBoxRef.current) setSelectionBox(null);
                if (selectedNodeIdsRef.current.size) setSelectedNodeIds(new Set());
                setSelectedConnectionId((current) => (current ? null : current));
                return;
            }
    ...
    const handleNodeMouseDown = useCallback((event: ReactMouseEvent, nodeId: string) => {
        ...
        setToolbarNodeId((current) => (current === null ? current : null));
        setSelectedConnectionId((current) => (current ? null : current));
        ...
        if (!areSetsEqual(currentSelected, nextSelected)) setSelectedNodeIds(nextSelected);
        ...
        dragRef.current = {
            ...
            initialSelectedNodes,
            initialSelectedNodeMap: new Map(initialSelectedNodes.map(({ id, x, y }) => [id, { x, y }])),
        };
    ...
    const finishNodeDrag = useCallback((clientX?: number, clientY?: number) => {
        ...
        const initialPositionByNodeId = dragRef.current.initialSelectedNodeMap;
        ...
        if (dragRef.current.hasMoved && clientX != null && clientY != null) {
            setNodes((prev) =>
                prev.map((node) => {
                    const initial = initialPositionByNodeId.get(node.id);
        ...
        if (wasClick && clickedNodeId) {
            const clickedNode = nodeByIdRef.current.get(clickedNodeId);
            keepNodeToolbar(clickedNodeId);
            if (clickedNode?.type === CanvasNodeType.Text) {
                setDialogNodeId((current) => (current === clickedNodeId ? current : null));
            } else if (!clickedNode?.metadata?.content) {
    ...
    const handleGlobalMouseMove = useCallback(
        (event: MouseEvent) => {
            ...
            if (connectingParamsRef.current && !pendingConnectionCreateRef.current) {
                ...
                scheduleMouseWorld(screenToCanvas(event.clientX, event.clientY));
            }
        },
        [finishNodeDrag, getConnectionDropTarget, scheduleMouseWorld, screenToCanvas],
    );
    ...
    const handleGlobalPointerMove = useCallback(
        (event: PointerEvent) => {
            ...
            visibleInteractionNodesRef.current.forEach((node) => {
        ...
        (event: MouseEvent) => {
            ...
            if (currentConnection) {
                ...
                if (dropTarget.nodeId) {
                    ...
                    const world = screenToCanvas(event.clientX, event.clientY);
                    scheduleMouseWorld(world);
                    setPendingConnectionCreate({ connection: currentConnection, position: world });
                }
            }
        },
        [connectNodes, finishNodeDrag, getConnectionDropTarget, scheduleMouseWorld, screenToCanvas, setConnecting],
    );
    ...
    useEffect(() => {
        ...
        return () => {
            ...
            if (mouseWorldRafRef.current) cancelAnimationFrame(mouseWorldRafRef.current);
        };
    ...
    const attachAssistantReferenceFile = useCallback(
        async (file: File, role: DirectorReferenceRole): Promise<DirectorReferencePackItem | null> => {
    ...
    const handleGenerateNode = useCallback(
        async (nodeId: string, mode: CanvasNodeGenerationMode, prompt: string) => {
            ...
            if (markSourceStatus) commitNodes((prev) => prev.map((node) => (node.id === nodeId ? { ...node, metadata: { ...node.metadata, prompt: statusPrompt, status: NODE_STATUS_LOADING, errorDetails: undefined } } : node)));
            ...
            try {
                if (mode === "image") {
                    ...
                    commitConnections((prev) => [...prev, ...batchConnections]);
                    ...
                    await Promise.all(
                        targetIds.map(async (targetId) => {
                            try {
                                ...
                                const errorDetails = error instanceof Error ? error.message : "生成失败";
                                hasFailure = true;
                                commitNodes((prev) => prev.map((node) => (node.id === targetId ? { ...node, metadata: { ...node.metadata, status: NODE_STATUS_ERROR, errorDetails } } : node)));
                                return false;
                            }
                        }),
                    );
                ...
                if (mode === "video") {
                    ...
                    commitNodes((prev) => (isEmptyVideoNode ? prev.map((node) => (node.id === nodeId ? { ...node, ...videoNode } : node)) : [...prev.map((node) => (node.id === nodeId ? { ...node, metadata: { ...node.metadata, status: NODE_STATUS_SUCCESS } } : node)), videoNode]));
                    if (!isEmptyVideoNode) commitConnections((prev) => [...prev, { id: nanoid(), fromNodeId: nodeId, toNodeId: videoId }]);
                ...
                if (mode === "audio") {
                    ...
                    commitNodes((prev) => (isEmptyAudioNode ? prev.map((node) => (node.id === nodeId ? { ...node, ...audioNode } : node)) : [...prev.map((node) => (node.id === nodeId ? { ...node, metadata: { ...node.metadata, status: NODE_STATUS_SUCCESS } } : node)), audioNode]));
                    if (!isEmptyAudioNode) commitConnections((prev) => [...prev, { id: nanoid(), fromNodeId: nodeId, toNodeId: audioId }]);
                    ...
                    commitNodes((prev) => prev.map((node) => (node.id === audioId ? { ...node, metadata: { ...node.metadata, ...audioMetadata(audio), prompt: effectivePrompt, ...buildAudioGenerationMetadata(generationConfig) } } : node)));
                    return;
                }
                ...
                if (isConfigNode || editingTextNode) {
                    ...
                    commitNodes((prev) => [...prev.map((node) => (node.id === nodeId && isConfigNode ? { ...node, metadata: { ...node.metadata, prompt: effectivePrompt, status: NODE_STATUS_LOADING, errorDetails: undefined } } : node)), ...childNodes]);
                    commitConnections((prev) => [...prev, ...childIds.map((childId) => ({ id: nanoid(), fromNodeId: nodeId, toNodeId: childId }))]);
                }
                ...
                const answers = await Promise.all(
                    (childIds.length ? childIds : [nodeId]).map((targetNodeId) => {
                        ...
                        return requestImageQuestion(generationConfig, buildNodeChatMessages({ ...generationContext, prompt: effectivePrompt }), (text) => {
                            ...
                            commitNodes((prev) => prev.map((node) => (node.id === targetNodeId ? { ...node, type: CanvasNodeType.Text, metadata: { ...node.metadata, content: text, status: NODE_STATUS_LOADING } } : node)));
        ...
        [commitConnections, commitNodes, effectiveConfig, isAiConfigReady, markNodeRunning, message, openConfigDialog],
    );

    const handleApplyDirectorWorkflow = useCallback(
        async (workflow: DirectorWorkflow): Promise<DirectorWorkflowMaterialization> => {
            const referenceNodeIds = Array.from(new Set([...(workflow.referencePack || []).map((item) => item.nodeId), ...workflow.references.map((reference) => reference.id)])).filter((id) => nodesRef.current.some((node) => node.id === id));
            ...
            const nextNodes = [...nodesRef.current, ...result.nodes];
            const nextConnections = [...connectionsRef.current, ...result.connections];
            nodesRef.current = nextNodes;
            connectionsRef.current = nextConnections;
            setNodes(nextNodes);
            setConnections(nextConnections);
            setSelectedNodeIds(new Set(result.materialization.executableNodeIds.length ? result.materialization.executableNodeIds : result.materialization.nodeIds));
            setSelectedConnectionId(null);
            setDialogNodeId(result.materialization.executableNodeIds[0] || result.materialization.firstNodeId || null);
            message.success("导演工作流已搭建到画布");
            return result.materialization;
        },
        [effectiveConfig, getCanvasCenter, message],
    );

    const handleExecuteDirectorWorkflow = useCallback(
        async (materialization: DirectorWorkflowMaterialization): Promise<DirectorWorkflowRunReport> => {
            let executedCount = 0;
            let skippedCount = 0;
            let failedCount = 0;
            let missingConfig = false;
            const executableNodes = materialization.executableNodeIds
                .map((nodeId) => nodesRef.current.find((node) => node.id === nodeId))
                .filter((node): node is CanvasNodeData => Boolean(node && node.type === CanvasNodeType.Config))
                .sort((first, second) => (first.metadata?.director?.plannedOrder || 0) - (second.metadata?.director?.plannedOrder || 0));
            ...
            for (const node of executableNodes) {
                ...
                setNodes((prev) => {
                    const next = prev.map((item) => (item.id === node.id ? withDirectorRunState(item, "running") : item));
                    nodesRef.current = next;
                    return next;
                });
                await handleGenerateNode(node.id, mode, prompt);
                const latest = nodesRef.current.find((item) => item.id === node.id);
                const failed = latest?.metadata?.status === NODE_STATUS_ERROR;
                if (failed) failedCount += 1;
                else executedCount += 1;
                setNodes((prev) => {
                    const next = prev.map((item) => (item.id === node.id ? withDirectorRunState(item, failed ? "error" : "done") : item));
                    nodesRef.current = next;
                    return next;
                });
            }

            const report = { workflowId: materialization.workflowId, executedCount, skippedCount, failedCount };
            if (missingConfig) {
                openConfigDialog(true);
                message.warning("部分节点缺少对应类型的渠道配置，已跳过");
            }
            if (executedCount || skippedCount) message.success(`工作流执行完成：${executedCount} 个已执行，${skippedCount} 个已跳过`);
            if (failedCount) message.warning(`工作流有 ${failedCount} 个节点失败，请在画布上查看错误详情`);
            return report;
        },
    ...
    const handleRetryNode = useCallback(
        async (node: CanvasNodeData) => {
            ...
            try {
                ...
                markNodeRunning(node.id, false);
            }
        },
        [effectiveConfig, isAiConfigReady, markNodeRunning, message, openConfigDialog],
    );
    ...
    const handleNodePreview = useCallback((node: CanvasNodeData) => {
        setPreviewNodeId(node.id);
    }, []);

    const handleRetryNodeClick = useCallback(
        (node: CanvasNodeData) => {
    ...
    const renderCanvasNodePanel = useCallback(
        (panelNode: CanvasNodeData) =>
            panelNode.type === CanvasNodeType.Config ? (
                ...
                <CanvasNodePromptPanel
                    ...
                    mentionReferences={mentionReferencesByNodeId.get(panelNode.id) || EMPTY_CANVAS_MENTION_REFERENCES}
        ...
        [configInputsById, handleConfigNodeChange, handleGenerateNode, handleNodePromptChange, mentionReferencesByNodeId, runningNodeIds],
    );
    ...
    const renderCanvasNodeContent = useCallback(
        ...
        [configInputsById, handleConfigNodeChange, handleGenerateNode, runningNodeIds],
    );

    if (!projectLoaded) return <CanvasRefreshShell />;

    return (
        <main className="flex h-full min-h-0 overflow-hidden bg-transparent" style={{ color: theme.node.text }}>
            <section className="relative min-w-0 flex-1 overflow-hidden">
                ...
                >
                    <svg className="absolute left-0 top-0 h-[10000px] w-[10000px] overflow-visible" style={{ pointerEvents: "none", transform: "translateZ(0)", zIndex: 0 }}>
                        <ConnectionLayer items={regularConnectionItems} />
                        {regularConnectionItems.map((connection) => (
                            <path
                                key={connection.id}
                                ...
                                onContextMenu={(event) => {
                                    event.preventDefault();
                                    event.stopPropagation();
                                    handleConnectionContextMenu?.(event, connection.id);
                                }}
                            />
                        ))}
                    ...
                    {visibleNodePresentation.map(({ node, compactContent }) => {
                        return (
                            <CanvasNode
                                key={node.id}
                                data={node}
                                ...
                                resourceLabel={compactContent ? undefined : resourceReferenceByNodeId.get(node.id)}
                                mentionReferences={compactContent ? EMPTY_CANVAS_MENTION_REFERENCES : mentionReferencesByNodeId.get(node.id) || EMPTY_CANVAS_MENTION_REFERENCES}
                    ...
                    {pendingConnectionCreate ? <ConnectionCreateMenu pending={pendingConnectionCreate} viewport={viewport} onCreate={(type) => createConnectedNode(type, pendingConnectionCreate)} onClose={cancelPendingConnectionCreate} /> : null}
                </InfiniteCanvas>
                ...
                {isMiniMapOpen ? <Minimap items={minimapItems} viewport={viewport} viewportSize={size} onViewportChange={handleViewportChange} /> : null}
                ...
                <Modal
                    ...
                    title={
                        <div>
                            <div className="text-base font-semibold text-[color:var(--sacred-on-surface)]">AI 超分</div>
                            <div className="mt-1 text-xs font-normal text-[color:var(--sacred-on-surface-variant)]">当前为预留能力，占位状态保持不变</div>
                        </div>
                    }
                    ...
                    styles={{ body: { maxHeight: "min(72vh, 520px)", overflowY: "auto" } }}
                >
                    <div className="canvas-image-tool-shell sacred-panel-soft py-8 text-center text-base font-medium text-[color:var(--sacred-on-surface-variant)]">暂未实现</div>
                </Modal>
                ...
                <Modal
                    ...
                    title={
                        <div>
                            <div className="text-base font-semibold text-[color:var(--sacred-on-surface)]">图片详情</div>
                            <div className="mt-1 text-xs font-normal text-[color:var(--sacred-on-surface-variant)]">查看当前节点生成图</div>
                        </div>
                    }
                ...
                >
                    {previewNode?.metadata?.content ? (
                        <div className="canvas-image-preview-frame">
                            <img
                ...
                <Modal
                    className="canvas-clear-confirm-modal"
                    title={
                        <div>
                            <div className="text-base font-semibold text-[color:var(--sacred-on-surface)]">清空画布？</div>
                            <div className="mt-1 text-xs font-normal text-[color:var(--sacred-on-surface-variant)]">当前画布内容将被移除</div>
                        </div>
                    }
                    ...
                    footer={
                        <div className="canvas-clear-confirm-actions">
                            <Button autoInsertSpace={false} onClick={() => setClearConfirmOpen(false)}>
                                <span>取消</span>
                            </Button>
                            <Button autoInsertSpace={false} danger type="primary" onClick={clearCanvas}>
                                <span>清空</span>
                            </Button>
                        </div>
                    }
                >
                    <div className="canvas-clear-confirm-body sacred-panel-soft">
                        <p className="m-0 text-sm text-[color:var(--sacred-on-surface-variant)]">这会删除当前画布上的所有节点和连线。</p>
                    </div>
                </Modal>
...
function CanvasTopBar({
    ...
    return (
        <>
            <div className="canvas-detail-topbar pointer-events-none absolute left-0 right-0 top-0 z-50 flex h-16 items-center justify-between px-4">
                <div className="canvas-detail-topbar-left pointer-events-auto flex min-w-0 items-center gap-3">
                    <Dropdown
                        trigger={["click"]}
                        menu={{
                            items: [
                    ...
                    <div ref={titleRef} className="canvas-detail-title-wrap flex min-w-0 items-center gap-2">
                        {isTitleEditing ? (
                            <input
                                autoFocus
                                value={titleDraft}
                                ...
                                className="canvas-detail-title-input max-w-[280px] bg-transparent p-0 text-left text-lg font-semibold tracking-normal outline-none"
                                style={{ color: theme.node.text }}
                            />
                        ) : (
                            <button
                                type="button"
                                className="canvas-detail-title-button max-w-[280px] truncate border-b border-dashed border-transparent text-left text-lg font-semibold tracking-normal transition hover:border-current"
                ...
                <div className="canvas-detail-topbar-actions pointer-events-auto flex items-center gap-1.5">
                    <UserStatusActions
                        variant="canvas"
            ...
            <Modal
                ...
                width={560}
                centered
                styles={{ body: { maxHeight: "min(72vh, 640px)", overflowY: "auto" } }}
            >
...
function Shortcut({ keys, value }: { keys: string[]; value: string }) {
    return (
        <div className="grid gap-2 rounded-lg px-2 py-1.5 transition-colors hover:bg-[rgba(var(--sacred-gold-rgb),0.08)] sm:grid-cols-[minmax(0,1fr)_140px] sm:items-center sm:gap-6">
            ...
            <span className="break-words text-sm text-[color:var(--sacred-on-surface-variant)] sm:text-right">{value}</span>
        </div>
    );
}
...
function withDirectorRunState(node: CanvasNodeData, runState: NonNullable<CanvasNodeMetadata["director"]>["runState"]): CanvasNodeData {
    if (!node.metadata?.director) return node;
    return {
        ...node,
        metadata: {
            ...node.metadata,
...
function useStableStringSet(value: Set<string>) {
    const ref = useRef(value);
    if (!areSetsEqual(ref.current, value)) {
        ref.current = value;
    }
    return ref.current;
}

function useStableNodeArray(value: CanvasNodeData[]) {
    const ref = useRef(value);
    if (!areNodeArraysEqual(ref.current, value)) {
        ref.current = value;
    }
    return ref.current;
}

function shouldUseCompactNodeContent(
    node: CanvasNodeData,
    ...
    if (isSelected || isRelated || isFocusRelated || isConnectionTarget || isConnecting || showPanel || isEditing) return false;
    if (viewportScale >= 0.78) return false;

    const projectedWidth = node.width * viewportScale;
    const projectedHeight = node.height * viewportScale;
    return projectedWidth < 220 && projectedHeight < 180;
}

function areNodeArraysEqual(first: CanvasNodeData[], second: CanvasNodeData[]) {
    if (first === second) return true;
...
function getHiddenBatchChildIds(nodes: CanvasNodeData[], collapsingBatchIds?: Set<string>) {
    ...
    nodes.forEach((node) => {
        ...
        if (!rootId) return;
        const root = nodeById.get(rootId);
        if (root && collapsingBatchIds?.has(rootId)) return;
        if (root && !root.metadata?.imageBatchExpanded) hiddenIds.add(node.id);
    });
    return hiddenIds;
}
