...
import { Copy, FolderOpen, History, KeyRound, Link2, LoaderCircle, PanelRightClose, PlugZap, Plus, RefreshCw, RotateCcw, Sparkles, Terminal, Trash2 } from "lucide-react";
import { motion } from "motion/react";

import { canvasThemes } from "@/lib/canvas-theme";
import { useThemeStore } from "@/stores/use-theme-store";
import { useUserStore } from "@/stores/use-user-store";
import { useCanvasAgentStore, type AgentAttachment, type AgentChatItem, type AgentEventLog, type AgentPanelTab, type AgentPendingToolCall, type AgentProvider, type AgentThreadSummary } from "../stores/use-canvas-agent-store";
...
const AGENT_CONNECT_STEPS = [
    ...
    { title: "2. 构建 CoGenesis Canvas Agent", text: "在本地项目的 canvas-agent 目录执行一次构建。", command: "npm install && npm run build" },
    { title: "3. 启动本地 Agent", text: "启动后终端会输出 Local URL 和 Connect token。", command: "node dist/index.js" },
...
export function CanvasLocalAgentPanel({
    ...
    const { width, url, token, connected, enabled, prompt, attachments, sending, waiting, messages, eventLogs, threads, activeThreadId, workspacePath, loadingThreads, activeTab, agentProvider, confirmTools, activity, connectError, pendingTool, setAgentState, addMessage: pushMessage, addEventLog: pushEventLog, clearEventLogs } = useCanvasAgentStore();
    ...
    const sharedContext = useMemo(() => {
        ...
        return {
            ...
            assets: (snapshot.assetPack || []).length,
            queue: queue.length,
            ready: queue.filter((item) => item.runState === "ready" || item.runState === "planned").length,
    ...
    const sendPrompt = async () => {
        ...
        try {
            const provider = useCanvasAgentStore.getState().agentProvider;
            const threadId = provider === "codex" ? useCanvasAgentStore.getState().activeThreadId || undefined : undefined;
            const res = await fetch(`${endpoint}/agent/${provider}/turn?token=${encodeURIComponent(token)}`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ prompt: requestPrompt, canvasId: snapshotRef.current.projectId, threadId, attachments: files.map(({ name, type, dataUrl }) => ({ name, type, dataUrl })) }) });
            if (!res.ok) throw new Error("本地 Agent 拒绝了请求");
            const data = (await res.json()) as { threadId?: string };
            if (provider === "codex" && data.threadId) setAgentState({ activeThreadId: data.threadId });
            addEventLog(`${agentProviderLabel(provider)} 已接收`, { status: res.status });
            files.forEach((item) => {
    ...
    const collapse = () => {
        onCollapseStart?.();
        window.setTimeout(() => onCollapse?.(), PANEL_MOTION_SECONDS * 1000);
    };
    ...
    return (
        <motion.div
            ...
            style={{ display: collapsed ? "none" : undefined, overflow: "clip", pointerEvents: collapsed ? "none" : undefined }}
        >
            <motion.aside
...
function AgentSharedContextSummary({
    theme,
    stats,
    agentProvider,
    ...
    const items = [
        ...
        { label: "连线", value: stats.connections },
            ...
            <div className="flex flex-wrap items-center gap-1.5">
                ...
                <div className="ml-auto flex items-center gap-2">
                    ...
                    <Segmented
                        ...
                        onChange={(value) => onAgentProviderChange(value as AgentProvider)}
                    />
                </div>
            </div>
...
function formatAgentEvent(event: AgentEventPayload): Omit<AgentChatItem, "id"> | null {
    ...
    if ((event.type === "item.updated" || event.type === "item.completed") && item?.type === "agent_message") return { role: "assistant", title: agentName(event), text: stringText(item.text), meta: usageText(event), streamId: item.id };
    ...
    if (text) return { role: "assistant", title: agentName(event), text, meta: usageText(event) };
    return null;
}
...
function eventTitle(event: AgentEventPayload) {
    ...
    if (event.type === "thread.started") return `已创建 ${agent} 会话`;
    ...
    if (event.type === "item.completed" && item?.type === "agent_message") return `${agent} 回复`;
    return event.type || `${agent} 事件`;
}
...
function agentName(event: AgentEventPayload) {
    return event.agent === "claude" ? "Claude" : "Codex";
}

function agentProviderLabel(provider: AgentProvider) {
